...
const resize = async (img) => {
	try { 
		const image = await sharp(img, { limitInputPixels: false }).resize(50, 50);
...
const addMem =  async(img) => {
  ...
  const image = await sharp(img, { limitInputPixels: false }).composite([{input: svgBuffer, left: 1150, top: 90}]);
  return image;
}
